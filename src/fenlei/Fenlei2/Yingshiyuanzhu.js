...
export default class Falv extends Component {
    ...
    componentDidMount() {
        fetch("http://101.201.252.4:3389/index",{
            method: 'POST', // or 'PUT'
            body:JSON.stringify({name:encodeURI('影視原著')}), // data can be `string` or {object}!
            headers: 
        ...
        return (
            ...
            <View style={{backgroundColor:'white'}}>
                <View style={styles.view1}>
                    ...
                    <Text style={styles.text1}>影視原著</Text>
                </View>
                {
                    this.state.totaldata.map(function(name){
                        var cat={new:name.herf};
                        return(
                            <TouchableOpacity style={styles.book} onPress={()=>Actions.tuijian2({'key':{new:name.herf}})}>
