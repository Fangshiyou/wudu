...
export default class fenleitushu extends Component {
    ...
    componentDidMount() {
        fetch('http://101.201.252.4:3389/xiezuoxs', {
        method: 'POST',
        mode: "cors",
        headers: { 'Content-Type': 'application/json'},
        body: JSON.stringify({
    ...
    render() {
        return (<ScrollView>
            ...
            <Text>书名</Text>
            <Text></Text>
        <TextInput style={styles.name}  multiline={true} onChangeText={this.showT}>
            {
                this.state.plarr.map((item, idx) =>
                <View>
                <Text style={styles.tex}>{item.bookname}</Text>
                </View>
                )
            }
            </TextInput></View>
            ...
            <Text></Text>
            <TextInput style={styles.name2}  multiline={true} onChangeText={this.showN}>
                {
                    this.state.plarr.map((item, idx) =>
                    <View>
                    <Text style={styles.tex}>{item.content}</Text>
                    </View>
                    )
                }
                </TextInput></View>
